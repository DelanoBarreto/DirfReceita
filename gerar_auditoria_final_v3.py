...
def gerar_relatorio():
    ...
    relatorio = []
    relatorio.append("=== RELATÓRIO DE AUDITORIA FINAL CONSOLIDADO - DIRF 2026 ===")
    relatorio.append("Este relatório compara as listas dos Planos de Saúde com o que foi importado.")
    relatorio.append("="*75 + "\n")
    ...
    try:
        ...
        # Identificar colunas manualmente pela posição ou nome similar
        ...
        relatorio.append(f"Ausentes na DIRF (Sem cadastro principal): {len(nao_importados_hap)}")
        if nao_importados_hap:
            relatorio.append("Alguns nomes ausentes (HAPVIDA):")
            relatorio.extend(nao_importados_hap[:30])
            if len(nao_importados_hap) > 30: relatorio.append(f"... e outros {len(nao_importados_hap)-30} nomes.")
    ...
    try:
        ...
        relatorio.append(f"Ausentes na DIRF (Sem cadastro principal): {len(nao_importados_odonto)}")
        if nao_importados_odonto:
            relatorio.append("Alguns nomes ausentes (ODONTOART):")
            relatorio.extend(nao_importados_odonto[:30])
            if len(nao_importados_odonto) > 30: relatorio.append(f"... e outros {len(nao_importados_odonto)-30} nomes.")
    except Exception as e:
    ...
    try:
        ...
        relatorio.append(f"Ausentes na DIRF (Sem cadastro principal): {len(nao_importados_clin)}")
        if nao_importados_clin:
            relatorio.append("Alguns nomes ausentes (CLIN):")
            relatorio.extend(nao_importados_clin[:30])
    except Exception as e:
        relatorio.append(f"Erro ao analisar CLIN: {e}")

    # 4. RESUMO DE ERROS (SINAL VERMELHO NO PGD)
    relatorio.append("\n" + "="*75)
    relatorio.append("!!! PENDÊNCIAS CRÍTICAS DE DADOS (ERROS NO PGD) !!!")
    relatorio.append("Estes 4 registros pertencem ao plano CLIN e precisam de correção manual:")
    relatorio.append("Eles estão no arquivo mas a Receita exige que você informe a DATA DE NASCIMENTO.")
    relatorio.append("-" * 75)
    pendencias = [
