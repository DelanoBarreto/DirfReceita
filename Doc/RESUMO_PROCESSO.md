...
| :--- | :--- | :--- | :--- |
| `processa_dirf_odontoart.py` | `Base/odontoart 2025.pdf` | Extração via Regex do PDF da Odontoart. | **Testado e Aprovado** no PGD. |
| `processa_dirf_odontoart_xls.py` | `Base/Odontoart 2025.xls` | Processamento do Excel (HTML) da Odontoart. | **Aguardando Validação Final** (3 testes). |
| `processa_dirf_saude_odonto.py` | `Base/SAUDE E ODONTO ATUALIZADO.xlsx` | Processamento da planilha consolidada (HAPVIDA + Outros). | **Aguardando Validação Final** (3 testes). |
...
1.  **Limpeza no PGD:** Ao importar os novos arquivos de teste (`2026_processado_xls.txt` ou `2026_processado_saude_odonto.txt` dentro de `Importação/`), use a opção **"Substituir"** ou exclua a declaração anterior no programa da Receita Federal para evitar erros de "Titular já existente".
...
-   `Importação/2026.txt`: Arquivo original validado.
-   `Importação/2026_processado_saude_odonto.txt`: Último arquivo gerado para testes da Hapvida/Saúde.
-   `processa_dirf_saude_odonto.py`: Script principal para a planilha consolidada.
-   `Doc/DOCUMENTACAO_DIRF.md`: Lições aprendidas e detalhes técnicos da arquitetura.
